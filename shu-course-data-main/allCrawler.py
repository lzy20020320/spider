...
class CrawlerSession:
    ...
    async def crawl(self):
        ...
        while (page - 1) * PAGE_SIZE < total:
            ...
            page += 1
            await asyncio.sleep(0.5)
        cours.sort(key=lambda x: (x['courseId'], x['teacherId']))
...
async def do_crawl(output_dir, backend, username, password):
    ...
    try:
        ...
        for term_id, data in result.items():
            with open(os.path.join(output_dir, os.path.join('terms', f'{term_id}.json')), 'w', encoding='utf-8') as fp:
                print(f'Save term data to {os.path.join("personal","terms", f"{term_id}.json")}')
